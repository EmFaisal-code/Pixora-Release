...
const FFmpegBridge = {
    ...
    _getFromIndexedDB(id) {
        return new Promise((resolve, reject) => {
            ...
            req.onsuccess = e => {
                const db = e.target.result;
                if (!db.objectStoreNames.contains('videos')) { db.close(); return resolve(null); }
    ...
    _deleteFromIndexedDB(id) {
        return new Promise((resolve, reject) => {
            ...
            req.onsuccess = e => {
                const db = e.target.result;
                if (!db.objectStoreNames.contains('videos')) { db.close(); return resolve(); }
                const tx = db.transaction('videos', 'readwrite');
                const delReq = tx.objectStore('videos').delete(id);
                delReq.onsuccess = () => { resolve(); db.close(); };
                delReq.onerror = () => { reject(delReq.error); db.close(); };
            };
            req.onerror = e => reject(e.target.error);
        });
    },
    ...
    async compress(arrayBuffer, progressCb) {
        ...
        return new Promise((resolve, reject) => {
            let initTimeout = setTimeout(() => {
                ...
                reject(new Error('Proses background tidak merespons (Timeout). Pastikan tab tidak tertidur atau silakan muat ulang ekstensi.'));
            }, 15000);
    ...
    async compressCustom(arrayBuffer, options, progressCb) {
        if (progressCb) progressCb(1, 'Menyiapkan proses kompresi background...');
        ...
        return new Promise((resolve, reject) => {
            ...
            const listener = (msg) => {
                if (msg.type === 'FFMPEG_PROGRESS' && msg.id === videoId) {
                    ...
                    chrome.runtime.onMessage.removeListener(listener);
                    this._getFromIndexedDB(videoId + '_out').then(async (outBuffer) => {
                        await this._deleteFromIndexedDB(videoId + '_out');
                        if (!outBuffer || outBuffer.byteLength < 1024) {
                            reject(new Error('Output kosong atau rusak. Coba format video MP4 yang berbeda.'));
                        } else {
                            resolve(outBuffer);
            ...
            chrome.runtime.onMessage.addListener(listener);
            chrome.runtime.sendMessage({ type: 'START_FFMPEG_CUSTOM_BG', id: videoId, options: options }, (res) => {
                if (chrome.runtime.lastError) {
                    chrome.runtime.onMessage.removeListener(listener);
                    this._deleteFromIndexedDB(videoId).catch(()=>{});
                    reject(new Error('Koneksi ke background processing gagal. Silakan muat ulang ekstensi.'));
...
function patchMp4V2(arrayBuffer) {
    try {
        ...
        // PIXORA METHOD: Add fake frames to confuse TikTok's re-encode pipeline.
        // Restored to aggressive 10x/5x multipliers. Since we fixed inject.js payload corruption,
        // these high multipliers will successfully bypass the transcoder (forcing TikTok to 
        // accept the original 1080p payload) without throwing a "Couldn't process" error.
        const FAKE_SAMPLE_SIZE = 8;
        let v2FakeSampleCount = realSampleCount * 9; // Default 10x total (9x fake)
        ...
        if (mdhd) {
            ...
            if (timescale > 0 && duration > 0) {
                ...
                if (fps > 45) {
                    // Video is likely 60fps. Use 5x multiplier (4x fake).
